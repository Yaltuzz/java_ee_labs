...
public class DataStore {
    ...
    public synchronized List<Office> findAllOfficesByOfficeCenter(int id) {
        OfficeCenter officeCenter = findOfficeCenter(id).get();
        return officeCenter.getOfficeList().stream()
                .map(CloningUtility::clone)
                .collect(Collectors.toList());
    }
    ...
    public synchronized void updateOfficeCenter(OfficeCenter officeCenter) throws IllegalArgumentException {
        findOfficeCenter(officeCenter.getId()).ifPresentOrElse(
                ...
                () -> {
                    throw new IllegalArgumentException(
                            String.format("The character with id does not exist"));
                });
    }
